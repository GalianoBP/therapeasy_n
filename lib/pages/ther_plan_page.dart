...
class _TherPlanPage extends State<TherPlanPage> {
  ...
  void initState() {
    myChannel.on(
      ...
      (payload, [ref]) async {
        bool ref = false;
        print(payload['new']['state']);
        if (payload['new']['state'] != payload['old']['state']) {
          ref = true;
        }
        (ref) ? setState(() {}) : () {};
      },
            ...
            RichText(
              ...
              text: TextSpan(
                style: TextStyle(
                    ...
                    fontSize: 25,
                    fontFamily: 'Gotham'),
                text: '${s.day}-${s.month}-${s.year} ${s.hour + 2}:${s.minute}',
              ),
            )
          ])),
      Flexible(
          ...
          child: Row(mainAxisAlignment: MainAxisAlignment.end, children: [
            ...
            IconButton(
              iconSize: 75,
              onPressed: (elem['state'] == 'wait')
                  ? () async {
                      print(await DbComms.supabase.from('ther_plan').update(
                          {'state': 'comp'}).eq('plan_id', elem['plan_id']));
                    }
                  : null,
